...
**Read it for yourself.** Each right can name documents in the pack's `references.json` (the Act, the illustrated Patients' Bill of Rights). After the right is quoted, the bot adds one line per document: what it is, whether it is law or policy, how big it is to open, and the link. A link is something a person acts on, so it is gated like a phone number: no recorded sign-off, no link, and a link that goes dead has its sign-off withdrawn. The bot links and never attaches, so a person on a weak connection chooses the cost. On the emergency branch there is one exception and one rule. The exception: when emergency care is being refused, the law itself is worth showing to the person refusing, so that escalation message can name a document and the bot sends `A1.reference` ("If it helps to show them, this is the law…", with the page the section is on). The rule: it comes after what to do and the numbers, never before, and no other emergency message carries a link. The line is plain text ending in the URL, so every channel can show it; the web page turns it into a link that sends no referrer.
...
| Someone keeps telling their story while the bot waits for a short answer (danger check, or "which hospital?") | A message of more than eight words that is not a known hospital, and does not start with yes or no, is more story. It goes to the classifier with everything said so far, so a late detail can still reach the emergency branch; it is never stored as the answer. The question is asked again and does not count towards the three. A NO to the danger check given before the new detail is set aside, and more detail never takes someone out of the emergency branch or repeats its steps. |
...
| A report is held for review | The receipt never says "counted" unless it is. Unknown hospital: "a person will check your report before it is counted". Possible duplicate, implausible, or AI unavailable: a deliberately vague "a person will review your report", so a spammer is not told what tripped. |
